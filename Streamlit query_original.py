...
def main():
    ...
    # Botón para reiniciar la encuesta (útil tras recargas)
    ...
    if current_question_key != FINAL_KEY:
        ...
        form_key = f"form_{current_question_key}"
        col1, col2 = st.columns([1,2])
        # Bouton Précédent (fuera del formulario para reaccionar inmediatamente)
        prev_question = get_prev_question(current_question_key, st.session_state.user_answers)
        with col1:
            if prev_question and st.button("Précédent ⬅️"):
                st.session_state.current_question = prev_question
        # Formulario para entrada y submit único (evita doble click)
        with col2:
        ...
        # Nota: ya no es necesario pedir doble clic.
    else:
